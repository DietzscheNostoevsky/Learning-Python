...
def format_address(address_string):
  ...
  # Separate the address string into parts
    instring = address_string.split()

    for num in instring:
        if num.isnumeric():
            housenumber.append(num)
    newhousenumber = str(housenumber[0])

    streetname = instring[1:]

    newstreetname = " ".join(streetname)

    return "house number {} on street named {}".format(newhousenumber, newstreetname)
